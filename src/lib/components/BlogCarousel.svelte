<script lang="ts">
  ...
  let slidesPerView = $derived(
    innerWidth < 768 ? 1 : innerWidth < 1024 ? 2 : Math.min(3, posts.length),
  );
  ...
  <div class={carouselWrapperStyles}>
    ...
    >
      {#each posts as post, index}
        <div
          class={slideContainerStyles}
          style="width: {100 / slidesPerView}%; flex: 0 0 {100 /
            slidesPerView}%;"
        >
          {#if post.isExternal}
            <a
              ...
              style="color: inherit; text-decoration: none; display: flex; width: 100%; height: 100%;"
            >
              <article
                class={cardStyles}
                style="width: 100%; margin: 0 auto; display: flex; flex-direction: column;"
              >
